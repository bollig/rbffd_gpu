...
namespace viennacl
{
  ...
  namespace linalg
  {
    ...
    ScalarType norm_2(VectorType< ScalarType > &v, int const& mpi_rank)
    {
        ScalarType norm = viennacl::linalg::norm_2(v);
        norm*=norm;
        ScalarType r;
        MPI_Allreduce(&norm, &r, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
        return r;
    }
#endif
    ...
    parallel_norm_2(VectorType const& v, int const& mpi_rank, typename VectorType::value_type& dummy)
    {
    ...
    //typename VectorType::value_type
    double
    parallel_norm_2(VectorType const& v, int const& mpi_rank, double& dummy)
    {
    ...
    template <typename VectorType>
    float
    parallel_norm_2(VectorType const& v, int const& mpi_rank, float& dummy)
    {
        float norm = viennacl::linalg::norm_2(v);
        norm *= norm;
        float r[1];

        MPI_Allreduce(&norm, r, 1, MPI_FLOAT, MPI_SUM, MPI_COMM_WORLD);
    ...
    template <typename VectorType>
    typename VectorType::value_type
    norm_2(VectorType const& v, int const& mpi_rank)
    {
        // Redirect the call to appropriate float or double routines (Type size matters with MPI)
        typename VectorType::value_type dummy;
        return parallel_norm_2(v, mpi_rank, dummy );
    }

    template < >
    double
    norm_2(boost::numeric::ublas::vector_range< boost::numeric::ublas::vector<double> > const& v, int const& mpi_rank)
    {
    ...
    template < >
    double
    norm_2(boost::numeric::ublas::vector_slice< boost::numeric::ublas::vector<double> > const& v, int const& mpi_rank)
    {
            double norm = boost::numeric::ublas::norm_2(v);
            norm *= norm;
            double r[1];
