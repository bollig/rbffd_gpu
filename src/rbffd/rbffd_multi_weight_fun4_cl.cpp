#include <stdlib.h>
#include <math.h>
#include "rbffd_multi_weight_fun4_cl.h"
#include "timer_eb.h"
...
//----------------------------------------------------------------------
//
RBFFD_MULTI_WEIGHT_FUN4_CL::RBFFD_MULTI_WEIGHT_FUN4_CL(DerTypes typesToCompute, Grid* grid, int dim_num, int rank)
   : RBFFD_CL(typesToCompute, grid, dim_num, rank)
{
    this->loadKernel("computeDerivMultiWeightFun4Kernel", "derivative_kernels.cl");
    this->allocateGPUMem();
...
//----------------------------------------------------------------------
void RBFFD_MULTI_WEIGHT_FUN4_CL::allocateGPUMem()
{
...
//----------------------------------------------------------------------
void RBFFD_MULTI_WEIGHT_FUN4_CL::applyWeightsForDerivDouble(unsigned int start_indx, unsigned int nb_stencils, double* u, double* deriv_x, double* deriv_y, double* deriv_z, double* deriv_l, bool isChangedU)
{
...
//----------------------------------------------------------------------
void RBFFD_MULTI_WEIGHT_FUN4_CL::updateWeightsDouble(bool forceFinish)
{
...
//
    ...
    if (weightsModified) {
        ...
        std::cout << "[RBFFD_MULTI_WEIGHT_FUN4_CL] Writing weights to GPU memory\n";
