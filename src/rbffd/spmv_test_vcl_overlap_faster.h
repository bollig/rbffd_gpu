...
{
    protected:
        RBFFD_VCL_OVERLAP_PROJECT* rbffd; 
        Domain* grid; 
        int rank, size;
    ...
    public: 
        SpMVTest(RBFFD_VCL_OVERLAP_PROJECT* r, Domain* domain, int mpi_rank=0, int mpi_size=0) : rbffd(r), grid(domain), rank(mpi_rank), size(mpi_size), sol_dim(1), disable_timers(0) {
            o_comm_size=0; 
            r_comm_size=0;
            setupTimers(); 
            allocateCommBuffers(); 
        ...
        void encodeSendBuf(VCL_VEC_t& gpu_vec) {
            ...
            VCL_CSR_MAT_t& DM_project_o = *(rbffd->getGPUProjectO());
            ...
            *encodedBuf = viennacl::linalg::prod(DM_project_o, gpu_vec); 
            if (!disable_timers) tm["encode_copy"]->start();
            viennacl::fast_copy(encodedBuf->begin(), encodedBuf->end(), this->sbuf);
        ...
        void decodeRecvBuf(VCL_VEC_t &gpu_vec) {
            ...
            // NOTE: ASSUME THAT R_BY_RANK IS LINEAR ON THE GPU AND WE KNOW WE RECIEVE
            // LINEAR FROM THE OTHER PROCS, SO WE CAN DIRECTLY COPY!
#if 0
            ...
            viennacl::fast_copy(this->rbuf,this->rbuf + set_R_size, setR.begin());  
