...
double ExactRegularGrid::laplacian(double x, double y, double z, double t)
{
    ...
    // NOTE: after using simpLapl and mylapl I found a nan was introduced when
    // calculaing the sin(..)/r when r=0 (it should short circuit because
    // sin(..)==0 so its 0/0 == 0. To bypass his nan we doing the convoluted
    // stuff below for "together"
