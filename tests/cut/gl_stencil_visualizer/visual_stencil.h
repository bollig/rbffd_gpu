#ifndef __VISUAL_STENCIL_H__
#define __VISUAL_STENCIL_H__
#include <vector> 
#include "Vec3.h"
...
{ 
	public: 
                // Each VisualStencil is given a set of coordinates
		// for nodes in the stencil and the weights associated with
		// each node. 
                VisualStencil(std::vector<Vec3> nodes, std::vector<double> weights);
                ~VisualStencil();
