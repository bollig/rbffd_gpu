...
#define _KDTREE_H_
// Evan Bollig (changes)
// Based on v2.0 (2012)  https://code.google.com/p/kdtree-matlab/
// Added: 
// 	- Point is Vec3 for rbffd_gpu support
// 	- ROOT macro is now KDTREE_ROOT (for openmp support)
// 	- custom distance routine for Vec3
// 	- custom constructor for double*
//	- fixed uint->int typecast (lines 283, 293)
//	- added ifndef _KDTREE_H_ 
// Removed:
//	- mex.h
...
class KDTree{
	...
	KDTree(const double* points, int nb_points, int dim); 
...
int build_recursively(vector< vector<int> >& sorter, vector<char>& sidehelper, int dim){
...
void linear_tree_print() const{
    for (unsigned int i=0; i < nodesPtrs.size(); i++) {
        Node* n = nodesPtrs[i];
        if(n==NULL) fprintf(stderr, "%d-th node is NULL.", i);
        if(n->isLeaf())
            printf("Node[%d] P[%d]\n",i,n->pIdx);
        else
            printf("Node[%d] key %.4f Children[%d %d]\n",i,n->key,n->LIdx,n->RIdx);
    }
}
