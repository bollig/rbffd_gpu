...
//----------------------------------------------------------------------
//----------------------------------------------------------------------
void DerivativeTests::testEigen(Grid& grid, int stencil_size, int nb_bnd, int tot_nb_pts)
{
    // read input file
...
#define USE_CONTOURSVD 1
    ...
    if (!this->weightsComputed) {
        ...
        cout << "WARNING! (this is not an error) DerivativeTests::computeAllWeights() was already called, not re-computing weights." << endl;
    }
}
//----------------------------------------------------------------------
