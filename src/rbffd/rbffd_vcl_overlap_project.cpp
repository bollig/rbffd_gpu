#include <stdlib.h>
#include <math.h>
#include "rbffd_vcl_overlap_project.h"
#include "timer_eb.h"
...
//----------------------------------------------------------------------
//
    RBFFD_VCL_OVERLAP_PROJECT::RBFFD_VCL_OVERLAP_PROJECT(DerTypes typesToCompute, Domain* grid, int dim_num, int rank)
...
//----------------------------------------------------------------------
//
void RBFFD_VCL_OVERLAP_PROJECT::setupTimers() {
    tm["loadAttach"] = new Timer("[RBFFD_VCL_OVERLAP_PROJECT] Load and Attach Kernel");
    tm["construct"] = new Timer("[RBFFD_VCL_OVERLAP_PROJECT] RBFFD_VCL_OVERLAP_PROJECT (constructor)");
    tm["computeDerivs"] = new Timer("[RBFFD_VCL_OVERLAP_PROJECT] computeRBFFD_s (compute derivatives using OpenCL");
    tm["sendWeights"] = new Timer("[RBFFD_VCL_OVERLAP_PROJECT]   (send stencil weights to GPU)");
    tm["applyWeights_cpu"] = new Timer("[RBFFD_VCL_OVERLAP_PROJECT] Test derivative of CPU Vector");
    tm["applyWeights_gpu"] = new Timer("[RBFFD_VCL_OVERLAP_PROJECT] Evaluate derivative on GPU");
}


//----------------------------------------------------------------------
//
void RBFFD_VCL_OVERLAP_PROJECT::loadKernel() {
    tm["construct"]->start();

    cout << "Inside RBFFD_VCL_OVERLAP_PROJECT constructor" << endl;
...
void RBFFD_VCL_OVERLAP_PROJECT::allocateGPUMem() {
    ...
    gpu_project_o = new VCL_CSR_MAT_t();
...
//----------------------------------------------------------------------
void RBFFD_VCL_OVERLAP_PROJECT::updateNodesOnGPU(bool forceFinish) {
    unsigned int nb_nodes = grid_ref.getNodeListSize();
...
void RBFFD_VCL_OVERLAP_PROJECT::clearCPUNodes() {
    delete [] cpu_nodes;
}


void RBFFD_VCL_OVERLAP_PROJECT::clearCPUWeights() {
...
void RBFFD_VCL_OVERLAP_PROJECT::updateWeightsDouble(bool forceFinish) {
    ...
    if (weightsModified) {
        ...
        std::cout << "[RBFFD_VCL_OVERLAP_PROJECT] Writing weights to GPU memory\n";
        ...
        int nb_o_stencils = 0;
        for (int ii = 0; ii < O_by_rank.size(); ii ++) {
            nb_o_stencils += O_by_rank[ii].size();
        }

        UBLAS_MAT_t cpu_project_o(nb_o_stencils, nb_nodes, nb_o_stencils * 1 );
        // Weights should be in csr format
        int kk = 0;
        for (int ii = 0; ii < O_by_rank.size(); ii ++) {
            for (int jj = 0; jj < O_by_rank[ii].size(); jj ++) {
                // TODO: need to get working with SOLDIM > 1
                cpu_project_o(kk, grid_ref.g2l(O_by_rank[ii][jj])) = 1.;
                kk++;
            }
        }
        viennacl::copy(cpu_project_o, *(gpu_project_o));
        std::cout << "nb_o_stencils = " << nb_o_stencils << "\n";
        std::cout << "project_o = " << gpu_project_o->size1() << ", " << gpu_project_o->size2() << ", " << gpu_project_o->nnz() << "\n";
...
//----------------------------------------------------------------------
//
void RBFFD_VCL_OVERLAP_PROJECT::updateFunctionDouble(unsigned int nb_nodes, double* u, bool forceFinish) {
...
//
//
void RBFFD_VCL_OVERLAP_PROJECT::applyWeightsForDerivDouble(DerType which, unsigned int nb_nodes, unsigned int nb_stencils, double* u, double* deriv, bool isChangedU)
{
...
void RBFFD_VCL_OVERLAP_PROJECT::applyWeightsForDeriv(DerType which, VCL_VEC_t& u, VCL_VEC_t& deriv, bool isChangedU) {
    std::cout << "[RBFFD_VCL_OVERLAP_PROJECT] Warning! Using GPU to apply weights, but NOT advance timestep\n";
