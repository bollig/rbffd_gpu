program gmres 
    ...
    !!--------------------
    !! bn = Start local index
    !! en = End local index
    loc_n = (n-1)/proc
    bn = 1+(my_rank)*loc_n
    en = bn + loc_n -1

    !! Barrier all for timing (ignore)
    ...
    do while ((rho>errtol).and.(m<mmax))
        ...
        if ((my_rank.gt.0).and.(my_rank.lt.proc-1)&
            ...
            !! Other procs must send RIGHT AND LEFT
            call mpi_send(x(0,en),(n+1),mpi_real,my_rank+1,50,&
                mpi_comm_world,ierr)
            call mpi_recv(x(0,en+1),(n+1),mpi_real,my_rank+1,50,&
                mpi_comm_world,status,ierr)
        ...
        !! r = b - A x_0     ( here they take into account that PDE is 2D and
        !                       has DM for each direction. )
        ...
        !! Apply preconditioner:    r = M^-1 (b - A x_0)
        ...
        !!  rho = sqrt(< r, r >)  ( Need a reduce norm_2 on this )
        ...
        !! g_{0} = rho  (first givens rotation)
        g(1) =rho
        ...
        do while((rho > errtol).and.(k < kmax))          
            ...
            !! v_i = -A v_(i-1)         ( Note: i think their product here accounts for
            !                                   stencil weights [-1 -1 4 -1 -1] )
