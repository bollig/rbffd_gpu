...
void computeOnCPU()
{
    ...
    der->RBFFD::computeDeriv(RBFFD::X, u_cpu, xderiv_cpu, true);
	exit(0);
    der->RBFFD::computeDeriv(RBFFD::Y, u_cpu, yderiv_cpu, false); // originally false
    der->RBFFD::computeDeriv(RBFFD::Z, u_cpu, zderiv_cpu, false); // orig false
    der->RBFFD::computeDeriv(RBFFD::LAPL, u_cpu, lderiv_cpu, false); // orig false
	tm["cpu_tests"]->end();
	#endif
