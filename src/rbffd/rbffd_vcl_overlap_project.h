#ifndef __RBFFD_VCL_OVERLAP_PROJECT_H__
#define __RBFFD_VCL_OVERLAP_PROJECT_H__
...
class RBFFD_VCL_OVERLAP_PROJECT : public RBFFD
{    protected: 
        ...
        // RBFFD_VCL_OVERLAP_PROJECT.h if you want these joined together. 
    ...
    public: 
        ...
        RBFFD_VCL_OVERLAP_PROJECT(DerTypes typesToCompute, Domain* grid, int dim_num, int rank=0);

        virtual ~RBFFD_VCL_OVERLAP_PROJECT() { 
            if (deleteCPUWeightsBuffer) { this->clearCPUWeights();} 
            if (deleteCPUNodesBuffer) { this->clearCPUNodes();}
            this->tm.printAll(); 
            this->tm.clear();
            std::cout << "RBFFD_VCL_OVERLAP_PROJECT destroyed\n";
        }; 
        ...
        // Get a permutation matrix that will project O into a sendbuf on the
        // GPU. 
        VCL_CSR_MAT_t* getGPUProjectO() { return gpu_project_o; }
