...
class KDTree {
    ...
    private: 
        int globalNodeIndx; // Index into nodesPtrs (EVAN BOLLIG 6/23/10)
...
//            t1->printAll();
        ...
        /**
         * Evan Bollig 6/21/10 (ADDED SUPPORT FOR double[] instead of vector<double>)
         * Creates a KDtree filled with the provided data.
         * 
         * @param points   a vector< Point > containing the point data
    ...
    public:
        ...
        void updateTree(double dpoints[], int nbpts, int dim_num) {
            ...
            for (int i = 0; i < this->npoints; i++) {
                for (int j = 0; j < this->ndim; j++) {
                    points[i][j] = dpoints[i * this->ndim + j];
                }
            }
    ...
    private:
        ...
        int updateTree_recursively(vector< vector<int> >& sortidx, vector<int>& pidx, int dim) {
#ifdef DEBUG
            cout << endl << endl << "received current vector: ";
            for (unsigned int i = 0; i < pidx.size(); i++)
                cout << pidx[ i ] << " ";
            ...
            node->pIdx = -1; //not a leaf
            node->key = points[ pivot ][ dim ];
            node->LIdx = updateTree_recursively(sortidx, Larray, (dim + 1) % ndim);
            node->RIdx = updateTree_recursively(sortidx, Rarray, (dim + 1) % ndim);
            return nodeIdx;
        }
    ...
    public:
        ...
        void left_depth_first_print(int nodeIdx = 0) {
        ...
        inline double distance_squared(double a[], const Point& b) {
            double d = 0;
            double N = b.size();
            for (int i = 0; i < N; i++)
                d += (a[i] - b[i])*(a[i] - b[i]);
    ...
    private:
        ...
        void check_border_distance(int nodeIdx, int dim, double pnt[], double& cdistsq, int& idx) {
            Node* node = nodesPtrs[ nodeIdx ];
