...
RegularGrid2D::RegularGrid2D(int n_x, int n_y, double minX, double maxX, double minY, double maxY, int stencil_size) : Grid(n_x, n_y, stencil_size) {
    ...
    dz = 0.; //(nb_z > 1) ? (zmax - zmin) / (nb_z - 1.) : 0.;
...
/*----------------------------------------------------------------------*/
void RegularGrid2D::generateGrid() {
    // Create a regular perturbed grid
    printf("dx = %f\n", dx);
    printf("dy = %f\n", dy);
    printf("dz = %f\n", dz);

    nb_rbf = nb_x * nb_y; // HARDCODED 2D
    ...
    for (int k = 0; k < 1; k++) {       // HARDCODED 2D
...
void RegularGrid2D::generateGrid(const char* file, int nb_bnd, int npts) {
    float x, y, z;
