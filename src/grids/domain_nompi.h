#ifndef _DomainNoMPI_NOMPI_H_
#define _DomainNoMPI_NOMPI_H_
...
class DomainNoMPI : public Grid //, public MPISendable
{
    public: 		// Member Properties
        ...
        std::set<int> O;			// Stencil Centers that are OUTPUT to other DomainNoMPIs but might NOT depend on R
    ...
    public: 	// Member Functions: 
        ...
        void generateDecomposition(std::vector<DomainNoMPI*>& subdomains, int x_divisions, int y_divisions = 1, int z_divisions = 1); 
        void GEgenerateDecomposition(std::vector<DomainNoMPI*>& subdomains, int x_divisions, int y_divisions, int z_divisions=1) ;


        // Fill this DomainNoMPIs stencil and position sets based on the global set of RBF centers and stencils. 
...
//            std::cout << "==> " << inside << std::endl;
        ...
        virtual int sendUpdate(int my_rank, int receiver_rank) { std::cout << "[DomainNoMPI] sending node updates not allowed yet.\n"; return 0; }
        virtual int receiveUpdate(int my_rank, int sender_rank) { std::cout << "[DomainNoMPI] receiving node updates not allowed yet.\n"; return 0; }

        virtual int sendFinal(int my_rank, int receiver_rank) { std::cout << "[DomainNoMPI] nothing final to send\n"; return 0; }
        virtual int receiveFinal(int my_rank, int sender_rank) { std::cout << "[DomainNoMPI] nothing final to receive\n"; return 0; }
#endif
        ...
        // Dump a bit of ascii art to show the dependency graph for a DomainNoMPI
